...
temp_path = 'D:\\PycharmProjects\\TikTok\\user\\users.csv'
...
def downloadd(url, type):
    ...
    if len(url) > 0:
        ...
        if type == 'music':
            path = f'D:/PycharmProjects/TikTok/source/{type}/{uuid.uuid4().hex}.mp3'
            r = requests.get(url, headers=headers)
            with open(path, 'wb') as f:
                f.write(r.content)
                f.close()
        if type == 'avatar':
            path = f'D:/PycharmProjects/TikTok/source/{type}/{uuid.uuid4().hex}.png'
            r = requests.get(url, headers=headers)
            with open(path, 'wb') as f:
                f.write(r.content)
                f.close()
        if type == 'video':
            path = f'D:/PycharmProjects/TikTok/source/{type}/{uuid.uuid4().hex}.mp4'
            r = requests.get(url, headers=headers)
